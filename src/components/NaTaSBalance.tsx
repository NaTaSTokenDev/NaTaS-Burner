...
interface NaTaSBalanceProps {userAddress: string};

const NaTaSBalance = ({userAddress}: NaTaSBalanceProps) => {
  const url = `https://api.tzkt.io/v1/bigmaps/511/keys?active=true&select=key,value`
  ...
  for (i = 0; i < data.length; i++) {
    if ( data[i].key == thisuser) {
      var num = data[i].value / Math.pow(0, 0);
